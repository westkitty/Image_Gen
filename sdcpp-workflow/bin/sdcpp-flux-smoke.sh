#!/usr/bin/env bash
# sdcpp-flux-smoke.sh — bounded Flux smoke proof on BigMac.
# Proves one thing only: the staged Flux component set can render a real PNG.
# Does not download models, move files, or touch Turbo paths.
set -euo pipefail
...
RUN_DIR="$(make_run_dir flux-smoke)"
SDCPP_LOGFILE="$RUN_DIR/flux-smoke.log"
export SDCPP_LOGFILE

REPORT="$RUN_DIR/flux-smoke-report.md"
MANIFEST="$RUN_DIR/flux-smoke-manifest.json"
UI_CARD="$RUN_DIR/ui-run-card.md"
SMOKE_CACHE="$SDCPP_STATE_DIR/flux-smoke-cache.json"
LOCAL_PNG="$RUN_DIR/flux-smoke.png"
REMOTE_RUN_DIR="$REMOTE_OUTPUT_DIR/$(basename "$RUN_DIR")"
REMOTE_PNG="$REMOTE_RUN_DIR/flux-smoke.png"
REMOTE_LOG="$REMOTE_RUN_DIR/flux-smoke.log"
...
MODEL_PATH="$MODEL_ROOT/flux/flux1-schnell/flux1-schnell-fp8.safetensors"
VAE_PATH="$MODEL_ROOT/flux/shared/ae.safetensors"
CLIP_L_PATH="$MODEL_ROOT/flux/shared/clip_l.safetensors"
T5XXL_PATH="$MODEL_ROOT/flux/shared/t5xxl_fp16.safetensors"
MIN_MODEL_BYTES=$((1024 * 1024 * 1024))
MIN_VAE_BYTES=$((1024 * 1024))
MIN_CLIP_BYTES=$((1024 * 1024))
MIN_T5XXL_BYTES=$((1024 * 1024 * 1024))
WIDTH=512
HEIGHT=512
STEPS=4
...
BUILD_DIR=""
SDCLI=""
GUIDANCE_ARG="--guidance 3.5"
PREDICTION_ARG="--prediction flux_flow"
VAE_FORMAT_ARG="--vae-format flux"
SAMPLER_ARG="--sampling-method euler"
...
(manifest_path, smoke_cache_path, run_dir, status, created_at, finished_at, wall_elapsed, first_failed_gate,
 model_volume_path, model_root, model_path, vae_path, clip_l_path, t5xxl_path,
 remote_model_bytes, remote_vae_bytes, remote_clip_bytes, remote_t5xxl_bytes,
 build_dir, sdcli_path, width, height, steps, guidance_arg, prediction_arg,
 vae_format_arg, sampler_arg, remote_run_dir, remote_png, remote_log, local_png,
 png_bytes_value, png_valid, runtime_smoke_proven, prompt, report_prompt) = sys.argv[1:]

def _num_arg(arg, prefix):
    arg = arg.strip()
...
obj = {
    ...
    "vae_minimum_bytes": 1024 * 1024,
    "clip_l_minimum_bytes": 1024 * 1024,
    "t5xxl_minimum_bytes": 1024 * 1024 * 1024,
    "build_dir": build_dir or None,
    "sdcli_path": sdcli_path or None,
    "width": int(width),
    "height": int(height),
    "steps": int(steps),
    "guidance": _num_arg(guidance_arg, "--guidance"),
    "prediction": prediction_arg.split(' ', 1)[1] if prediction_arg.startswith('--prediction ') else None,
    "vae_format": vae_format_arg.split(' ', 1)[1] if vae_format_arg.startswith('--vae-format ') else None,
    "sampler": sampler_arg.split(' ', 1)[1] if sampler_arg.startswith('--sampling-method ') else None,
    ...
    "notes": "Bounded Flux smoke proof only. The current workflow uses the fp8 Flux candidate; SDXL Turbo remains a separate gate."
}
...
PYMANIFEST

  {
    ...
    printf '\n## Checkpoints\n\n'
    printf '%s\n' "- model: $MODEL_PATH" "- vae: $VAE_PATH" "- clip_l: $CLIP_L_PATH" "- t5xxl: $T5XXL_PATH"
    printf '\n## Binary\n\n'
    printf '%s\n' "- sd-cli: ${SDCLI:-unknown}" "- prompt: [REDACTED]" "- size: ${WIDTH}x${HEIGHT}" "- steps: $STEPS" "- guidance: ${GUIDANCE_ARG#--guidance }" "- prediction: ${PREDICTION_ARG#--prediction }" "- vae-format: ${VAE_FORMAT_ARG#--vae-format }" "- sampler: ${SAMPLER_ARG#--sampling-method }"
    printf '\n## Outputs\n\n'
    ...
    printf '\n## Result\n\n'
    if [ "$status" = "PASS" ]; then
      printf '%s\n' 'Flux smoke proof passed.'
    else
      printf '%s\n' "First failed gate: ${FIRST_FAILED_GATE:-unknown}"
      printf '%s\n' 'Flux smoke proof did not complete.'
    fi
  } > "$REPORT"
  ...
  if [ "$status" = "PASS" ]; then
    write_ui_run_card \
      ...
      "PASS" \
      "flux-smoke.png" \
      "flux-smoke-manifest.json" \
      "$REPORT_PROMPT" \
      "model=$MODEL_PATH vae=$VAE_PATH clip_l=$CLIP_L_PATH t5xxl=$T5XXL_PATH size=${WIDTH}x${HEIGHT} steps=$STEPS guidance=${GUIDANCE_ARG#--guidance }" \
      "$CREATED_AT" \
      >/dev/null
  fi
}
...
log "Starting bounded Flux smoke run in $RUN_DIR"
...
remote_test "test -s \"$MODEL_PATH\"" || smoke_fail "model-present" "Flux model is missing or empty: $MODEL_PATH"
remote_test "test -s \"$VAE_PATH\"" || smoke_fail "vae-present" "Flux VAE is missing or empty: $VAE_PATH"
remote_test "test -s \"$CLIP_L_PATH\"" || smoke_fail "clip-present" "Flux CLIP-L is missing or empty: $CLIP_L_PATH"
remote_test "test -s \"$T5XXL_PATH\"" || smoke_fail "t5-present" "Flux T5XXL is missing or empty: $T5XXL_PATH"

REMOTE_MODEL_BYTES="$(ssh_remote "stat -f %z \"$MODEL_PATH\" 2>/dev/null || wc -c < \"$MODEL_PATH\" 2>/dev/null || printf '0'" 2>&1 | tail -n 1 | tr -d '[:space:]')"
REMOTE_VAE_BYTES="$(ssh_remote "stat -f %z \"$VAE_PATH\" 2>/dev/null || wc -c < \"$VAE_PATH\" 2>/dev/null || printf '0'" 2>&1 | tail -n 1 | tr -d '[:space:]')"
REMOTE_CLIP_BYTES="$(ssh_remote "stat -f %z \"$CLIP_L_PATH\" 2>/dev/null || wc -c < \"$CLIP_L_PATH\" 2>/dev/null || printf '0'" 2>&1 | tail -n 1 | tr -d '[:space:]')"
REMOTE_T5XXL_BYTES="$(ssh_remote "stat -f %z \"$T5XXL_PATH\" 2>/dev/null || wc -c < \"$T5XXL_PATH\" 2>/dev/null || printf '0'" 2>&1 | tail -n 1 | tr -d '[:space:]')"

for pair in \
  "model:$REMOTE_MODEL_BYTES:$MIN_MODEL_BYTES" \
  "vae:$REMOTE_VAE_BYTES:$MIN_VAE_BYTES" \
  "clip:$REMOTE_CLIP_BYTES:$MIN_CLIP_BYTES" \
  "t5:$REMOTE_T5XXL_BYTES:$MIN_T5XXL_BYTES"
do
  kind="${pair%%:*}"
  rest="${pair#*:}"
  bytes="${rest%%:*}"
  ...
  case "$bytes" in
    ''|*[!0-9]*)
      smoke_fail "size-$kind" "Could not read a numeric size for $kind file."
      ;;
  esac
  if [ "$bytes" -lt "$min" ]; then
    smoke_fail "size-$kind" "Flux $kind file is too small (${bytes} bytes; need at least ${min})."
  fi
done
...
REMOTE_STDOUT_CMD="\"$SDCLI\" -m \"$MODEL_PATH\" --clip_l \"$CLIP_L_PATH\" --t5xxl \"$T5XXL_PATH\" --vae \"$VAE_PATH\" $VAE_FORMAT_ARG -p $Q_PROMPT -o \"$REMOTE_PNG\" -W $WIDTH -H $HEIGHT --steps $STEPS $GUIDANCE_ARG $PREDICTION_ARG $SAMPLER_ARG --diffusion-fa 2>&1 | tee \"$REMOTE_LOG\""
log "Running remote bounded Flux smoke on BigMac"
...
verify_png "$LOCAL_PNG" "Flux smoke PNG"
PNG_VALID="true"
RUNTIME_SMOKE_PROVEN="true"
RUN_STATUS="PASS"
...
pass_banner "Flux smoke proof complete.
Run: $RUN_DIR
PNG: $LOCAL_PNG
Report: $REPORT
Manifest: $MANIFEST"
