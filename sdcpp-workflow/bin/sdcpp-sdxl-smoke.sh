#!/usr/bin/env bash
# sdcpp-sdxl-smoke.sh — bounded SDXL base smoke proof on BigMac.
# Proves one thing only: the staged SDXL base checkpoint can render a real PNG.
# Does not download models, move files, or touch Flux / Turbo paths.
set -euo pipefail
...
RUN_DIR="$(make_run_dir sdxl-smoke)"
SDCPP_LOGFILE="$RUN_DIR/sdxl-smoke.log"
export SDCPP_LOGFILE

REPORT="$RUN_DIR/sdxl-smoke-report.md"
MANIFEST="$RUN_DIR/sdxl-smoke-manifest.json"
UI_CARD="$RUN_DIR/ui-run-card.md"
SMOKE_CACHE="$SDCPP_STATE_DIR/sdxl-smoke-cache.json"
LOCAL_PNG="$RUN_DIR/sdxl-smoke.png"
REMOTE_RUN_DIR="$REMOTE_OUTPUT_DIR/$(basename "$RUN_DIR")"
REMOTE_PNG="$REMOTE_RUN_DIR/sdxl-smoke.png"
REMOTE_LOG="$REMOTE_RUN_DIR/sdxl-smoke.log"
...
MODEL_PATH="$MODEL_ROOT/checkpoints/sdxl/sd_xl_base_1.0.safetensors"
...
(manifest_path, smoke_cache_path, run_dir, status, created_at, finished_at, wall_elapsed, first_failed_gate,
 ...
 width, height, steps, cfg_arg, remote_run_dir, remote_png, remote_log, local_png,
 png_bytes_value, png_valid, runtime_smoke_proven, prompt, report_prompt) = sys.argv[1:]

cfg_scale = None
cfg_arg = cfg_arg.strip()
if cfg_arg.startswith('--cfg-scale '):
    try:
        cfg_scale = float(cfg_arg.split(' ', 1)[1])
    except ValueError:
        cfg_scale = None
...
obj = {
    ...
    "notes": "Bounded SDXL base smoke proof only. Flux and SDXL Turbo remain blocked."
}
...
PYMANIFEST

  {
    ...
    printf '\n## Binary\n\n'
    printf '%s\n' "- sd-cli: ${SDCLI:-unknown}" "- prompt: [REDACTED]" "- size: ${WIDTH}x${HEIGHT}" "- steps: $STEPS" "- cfg: ${CFG_ARG#--cfg-scale }"
    printf '\n## Outputs\n\n'
    ...
    printf '\n## Result\n\n'
    if [ "$status" = "PASS" ]; then
      printf '%s\n' 'SDXL base smoke proof passed.'
    else
      printf '%s\n' "First failed gate: ${FIRST_FAILED_GATE:-unknown}"
      printf '%s\n' 'SDXL base smoke proof did not complete.'
    fi
  } > "$REPORT"
  ...
  if [ "$status" = "PASS" ]; then
    write_ui_run_card \
      ...
      "PASS" \
      "sdxl-smoke.png" \
      "sdxl-smoke-manifest.json" \
      "$REPORT_PROMPT" \
      "model=$MODEL_PATH size=${WIDTH}x${HEIGHT} steps=$STEPS cfg=${CFG_ARG#--cfg-scale }" \
      "$CREATED_AT" \
      >/dev/null
  fi
}
...
log "Starting bounded SDXL smoke run in $RUN_DIR"
...
remote_test "test -s \"$MODEL_PATH\"" || smoke_fail "model-present" "SDXL base checkpoint is missing or empty: $MODEL_PATH"
...
if [ "$REMOTE_MODEL_BYTES" -lt "$MIN_MODEL_BYTES" ]; then
  smoke_fail "model-size" "SDXL base checkpoint is too small (${REMOTE_MODEL_BYTES} bytes; need at least ${MIN_MODEL_BYTES})."
fi
...
if printf '%s\n' "$HELP_OUTPUT" | grep -q -- '--cfg-scale'; then
  CFG_ARG='--cfg-scale 7'
fi
...
REMOTE_STDOUT_CMD="\"$SDCLI\" -m \"$MODEL_PATH\" -p $Q_PROMPT -o \"$REMOTE_PNG\" -W $WIDTH -H $HEIGHT --steps $STEPS ${CFG_ARG:-} --diffusion-fa 2>&1 | tee \"$REMOTE_LOG\""
log "Running remote bounded smoke on BigMac"
...
verify_png "$LOCAL_PNG" "SDXL smoke PNG"
PNG_VALID="true"
RUNTIME_SMOKE_PROVEN="true"
RUN_STATUS="PASS"
...
pass_banner "SDXL base smoke proof complete.
Run: $RUN_DIR
PNG: $LOCAL_PNG
Report: $REPORT
Manifest: $MANIFEST"
