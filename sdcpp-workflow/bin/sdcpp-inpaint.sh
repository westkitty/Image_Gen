#!/usr/bin/env bash
# sdcpp-inpaint.sh — inpaint via sd-cli on BigMac.
# Conditions generation on an existing image + mask via --init-img + --mask + --strength.
# Init image must be within runs/; mask must be within runs/ or mask-uploads/.
set -euo pipefail
...
Usage: $(basename "$0") [options]
  ...
  --mask PATH          path to mask PNG (white=inpaint, black=keep; must be within workflow dir)
...
[ -z "$ARG_MASK" ] && fail "args" "--mask is required"
...
# ----- validate mask path ---------------------------------------------------
# Allow within runs/ (e.g. reusing an existing image as mask) or mask-uploads/.
WORKFLOW_DIR="$(dirname "$SDCPP_RUNS_DIR")"
MASK_ABS="$(cd "$(dirname "$ARG_MASK")" 2>/dev/null && pwd)/$(basename "$ARG_MASK")" \
  || fail "mask" "Cannot resolve mask path: $ARG_MASK"
case "$MASK_ABS" in
  "$SDCPP_RUNS_DIR/"*|"$WORKFLOW_DIR/mask-uploads/"*) ;;
  *) fail "mask" "Mask must be within $WORKFLOW_DIR/runs/ or $WORKFLOW_DIR/mask-uploads/ (got: $MASK_ABS)" ;;
esac
[ -f "$MASK_ABS" ] || fail "mask" "Mask file not found: $MASK_ABS"
...
RUN_DIR="$(make_run_dir inpaint)"
SDCPP_LOGFILE="$RUN_DIR/inpaint.log"; export SDCPP_LOGFILE
NAME="${ARG_OUT_NAME:-inpaint_$(timestamp)}"
LOCAL_PNG="$RUN_DIR/$NAME.png"
REPORT="$RUN_DIR/inpaint-run-report.md"
CREATED_AT="$(iso_now)"
...
cat > "$RUN_DIR/run-metadata.json" <<EOF
{
  ...
  "remote_mask_img": "$REMOTE_MASK_IMG",
...

{
  ...
  echo "- Remote mask: $REMOTE_MASK_IMG"
  echo "- Remote PNG: $REMOTE_PNG"
  echo "- Local PNG: $LOCAL_PNG"
  echo
} > "$REPORT"

# ----- upload init image and mask to BigMac ----------------------------------
...
scp "$INIT_IMG_ABS" "$SSH_TARGET:$REMOTE_INIT_IMG_ABS" >/dev/null \
  || fail "scp-init-img" "scp failed uploading init image: $INIT_IMG_ABS -> $REMOTE_INIT_IMG_ABS"
log "Init image uploaded: $REMOTE_INIT_IMG_ABS"

log "=== Uploading mask to BigMac ==="
REMOTE_MASK_IMG_ABS="$(remote_eval_path "$REMOTE_MASK_IMG")"
scp "$MASK_ABS" "$SSH_TARGET:$REMOTE_MASK_IMG_ABS" >/dev/null \
  || fail "scp-mask" "scp failed uploading mask: $MASK_ABS -> $REMOTE_MASK_IMG_ABS"
log "Mask uploaded: $REMOTE_MASK_IMG_ABS"

# ----- generate (remote) ----------------------------------------------------
log "=== Generating inpaint on BigMac (strength=$ARG_STRENGTH) ==="
...
if ! remote_test "test -s \"$REMOTE_PNG\" && file \"$REMOTE_PNG\" | grep -q 'PNG image data'"; then
  ...
  fail "inpaint-remote-png" "Remote PNG missing or invalid: $REMOTE_PNG"
fi
...
scp "$SSH_TARGET:$REMOTE_PNG_ABS" "$LOCAL_PNG" >/dev/null \
  || fail "inpaint-scp" "scp failed: $REMOTE_PNG_ABS -> $LOCAL_PNG"

# ----- verify locally + strip metadata when save_prompts=false ---------------
verify_png "$LOCAL_PNG" "inpaint PNG"
if [ "${SDCPP_REDACT_PROMPTS:-0}" = "1" ]; then
...
PNG_BYTES="$(png_bytes "$LOCAL_PNG")"
{
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$(sanitize_tsv "$BUILD_DIR")" "$(sanitize_tsv "$REMOTE_MODEL")" "inpaint" "custom" \
    ...
    "$(sanitize_tsv "$LOCAL_PNG")" "$PNG_BYTES" "$LOCAL_SUM" "cold" "n/a" "n/a" "inpaint-local" "PASS"
} > "$RUN_DIR/metrics.tsv"

# ----- UI run card ----------------------------------------------------------
PRIMARY_REL="$(basename "$LOCAL_PNG")"
write_ui_run_card "$RUN_DIR" "inpaint" "PASS" "$PRIMARY_REL" "run-metadata.json" \
  "$REPORT_PROMPT" \
...
pass_banner "INPAINT PASS (strength=$ARG_STRENGTH, seed=$SEED_LABEL, ${ELAPSED}s wall / ${REMOTE_ELAPSED}s remote).
Local PNG: $LOCAL_PNG
Report:    $REPORT"
